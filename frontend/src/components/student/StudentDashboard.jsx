...
const StudentDashboard = () => {
  ...
  const fetchDashboardMetrics = async () => {
    try {
      ...
      setAttendanceData(data.attendenceRecord);
  ...
  return (
    <section className="w-full px-2 mt-24 md:mt-32">
      ...
      />
      {generateQR ? (
        <GenerateQRPage setGenerateQR={setGenerateQR} />
      ) : scanQR ? <ScanQRPage setScanQR={setScanQR} /> : (
        <>
          ...
          <section className="panel_section">
            ...
            <AttendanceRecord data={attendanceData} />
          </section>
        </>
      )}
    </section>
